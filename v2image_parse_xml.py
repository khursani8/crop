...
#     xml_nameimg = os.path.join(
#         src_dirimgxml, os.path.basename(nameimg).rsplit(".")[0]+".xml")
...
for entry in xml_list:
    ...
    for haha in file_xml:
        if haha.tag == "object":
            ...
            write_image = cv2.cvtColor(
                write_image, cv2.COLOR_RGB2BGR)
            # h,w,c = img.shape
            save_name = ''.join(random.SystemRandom().choice(
                string.ascii_uppercase + string.digits) for _ in range(44))
            save_image_name = os.path.join(
                dir_save_img, save_name+".jpg")
